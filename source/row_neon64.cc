...
void I212ToARGBRow_NEON(const uint16_t* src_y,
  ...
  asm(YUVTORGB_SETUP
      ...
      : [src_y] "+r"(src_y), [src_u] "+r"(src_u), [src_v] "+r"(src_v),
        [dst_argb] "+r"(dst_argb), [width] "+r"(width)
      : [kUVCoeff] "r"(uv_coeff), [kRGBCoeffBias] "r"(rgb_coeff)
      : "cc", "memory", YUVTORGB_REGS, "v19");
}
...
void P210ToARGBRow_NEON(const uint16_t* src_y,
  ...
  asm volatile(
      YUVTORGB_SETUP
      "movi         v19.8b, #255                       \n"
      "ldr          q2, [%[kP210LoadShuffleIndices]]   \n"
      "1:                                              \n"  //
      READYUVP210 NVTORGB RGBTORGB8
      "subs         %w[width], %w[width], #8           \n"
      "st4          {v16.8b, v17.8b, v18.8b, v19.8b}, [%[dst_argb]], #32 \n"
      "b.gt         1b                         \n"
      : [src_y] "+r"(src_y), [src_uv] "+r"(src_uv), [dst_argb] "+r"(dst_argb),
        [width] "+r"(width)
      : [kUVCoeff] "r"(uv_coeff), [kRGBCoeffBias] "r"(rgb_coeff),
        [kP210LoadShuffleIndices] "r"(kP210LoadShuffleIndices)
      : "cc", "memory", YUVTORGB_REGS, "v19");
}
...
void P410ToARGBRow_NEON(const uint16_t* src_y,
  ...
  asm volatile(
      YUVTORGB_SETUP
      "movi         v19.8b, #255               \n"
      "ldr          q2, [%[kP410LoadShuffleIndices]]   \n"
      ...
      : [src_y] "+r"(src_y), [src_uv] "+r"(src_uv), [dst_argb] "+r"(dst_argb),
        [width] "+r"(width)
      : [kUVCoeff] "r"(uv_coeff), [kRGBCoeffBias] "r"(rgb_coeff),
        [kP410LoadShuffleIndices] "r"(kP410LoadShuffleIndices)
      : "cc", "memory", YUVTORGB_REGS, "v19");
}

...
int I010ToI410(const uint16_t* src_y,
  ...
  ret = ScalePlane_12(src_u, src_stride_u, SUBSAMPLE(width, 1, 1),
                      SUBSAMPLE(height, 1, 1), dst_u, dst_stride_u, Abs(width),
                      Abs(height), kFilterBilinear);
  if (ret != 0) {
    return ret;
  }
  ret = ScalePlane_12(src_v, src_stride_v, SUBSAMPLE(width, 1, 1),
...
int I422ToI444(const uint8_t* src_y,
  ...
  ret = ScalePlane(src_u, src_stride_u, SUBSAMPLE(width, 1, 1), height, dst_u,
                   dst_stride_u, Abs(width), Abs(height), kFilterBilinear);
  if (ret != 0) {
    return ret;
  }
  ret = ScalePlane(src_v, src_stride_v, SUBSAMPLE(width, 1, 1), height, dst_v,
                   dst_stride_v, Abs(width), Abs(height), kFilterBilinear);
  return ret;
}

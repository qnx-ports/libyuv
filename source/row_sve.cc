...
static inline void RAWToABCDRow_SVE2(const uint8_t* src_raw,
                                     uint8_t* dst_abcd,
                                     int width,
                                     uint32_t idx_start,
                                     uint32_t idx_step,
  ...
  asm("index   z31.s, %w[idx_start], %w[idx_step]        \n"
      "dup     z30.s, %w[alpha]                          \n"
      "subs     %w[width], %w[width], %w[vl]             \n"
      "b.lt     2f                                       \n"
...
void RAWToARGBRow_SVE2(const uint8_t* src_raw, uint8_t* dst_argb, int width) {
  RAWToABCDRow_SVE2(src_raw, dst_argb, width, 0xff000102U, 0x00030303U,
                    0xff000000U);
}

void RAWToRGBARow_SVE2(const uint8_t* src_raw, uint8_t* dst_rgba, int width) {
  RAWToABCDRow_SVE2(src_raw, dst_rgba, width, 0x000102ffU, 0x03030300U,
                    0x000000ffU);
}
...
static inline void ARGBToABCRow_SVE2(const uint8_t* src_argb,
                                     uint8_t* dst_abc,
                                     int width,
  ...
  uint32_t vl_mul3 = vl * 3;
  uint32_t rem_mul3;
  asm("whilelt p1.b, wzr, %w[vl_mul3]                     \n"
...
void ARGBToRGB24Row_SVE2(const uint8_t* src_argb, uint8_t* dst_rgb, int width) {
  ARGBToABCRow_SVE2(src_argb, dst_rgb, width, kARGBToRGB24RowIndices);
}

void ARGBToRAWRow_SVE2(const uint8_t* src_argb, uint8_t* dst_rgb, int width) {
  ARGBToABCRow_SVE2(src_argb, dst_rgb, width, kARGBToRAWRowIndices);
}
